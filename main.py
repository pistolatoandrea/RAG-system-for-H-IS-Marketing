...
@app.get("/")
def health_check():
    return {"status": "online", "knowledge_base": "loaded" if retriever else "empty"}
...
async def ask_ai(request: Request):
    ...
    try:
        ...
        # Prompt Multilingua e Context-Aware
        ...
        USER INTERESTS:
        The user is interested in: {program_info} 
        ...
        chain = (
            {"context": retriever | format_docs, "question": RunnablePassthrough(), "program_info": lambda x: selected_program}
            | prompt
            | model
            | StrOutputParser()
        )
